...
def get_result(strike, ball, how_many_user_tried):
    if (strike == 3):
        print("%dS %dB" % (strike, ball))
        print("축하합니다. %d번만에 세 숫자의 값과 위치를 모두 맞추셨습니다." % how_many_user_tried)
        return False
    print("%dS %dB" % (strike, ball))
    how_many_user_tried = how_many_user_tried + 1
    return how_many_user_tried
...
def baseball_3Strik_game():
    ...
    while True:
        ...
        result = get_result(result[0], result[1], how_many_user_tried)
        if (result == False):
            return False
        if (result):
            how_many_user_tried = result
